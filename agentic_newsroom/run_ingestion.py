...
def run():
    ...
    if not SKIP_INGESTION:
        try:
            ...
            articles_with_body = enrich_articles_with_body(rss_articles, delay=0.2)
    ...
    if SKIP_EXTRACTION:
        ...
        try:
            ...
            # Skip articles already embedded in vector DB — they were extracted last run
            try:
                _store_check = VectorStore()
                existing_ids = set()
                for a in prepared:
                    url = a.get("url","")
                    if url:
                        import hashlib
                        chunk_id = hashlib.sha256(f"{url}::0".encode()).hexdigest()[:32]
                        try:
                            res = _store_check.collection.get(ids=[chunk_id])
                            if res["ids"]:
                                existing_ids.add(url)
                        except Exception:
                            pass
                already_extracted = len(existing_ids)
                prepared_new = [a for a in prepared if a.get("url") not in existing_ids]
                logger.info(f"Extraction dedup: {already_extracted} articles already in vector DB, skipping")
            except Exception:
                prepared_new = prepared

            rss_only  = [a for a in prepared_new if a.get("type") == "rss_article"]
            non_rss   = [a for a in prepared_new if a.get("type") != "rss_article"]
